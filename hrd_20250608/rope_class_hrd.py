...
class rope_propagator:
    ...
    '''
    def __init__( self, datapath: str = ".", drivers = None, selected_bf_dict = None, delta_rho_ic = 0):
        #72: longitude intervals, 36: latitude intervals, 45: altitude intervals
        self.input_data_sindy = np.load(path.join(datapath, 'z_drivers_dataset_hrd_02_09_2norm_hourly_resolution_v4.npz'), allow_pickle=True)
        ...
        self.q_low_f10_value = self.input_data_sindy['models_coefficients'][()]['q_low_f10_value']
        self.q_high_f10_value = self.input_data_sindy['models_coefficients'][()]['q_high_f10_value']
    ...
    def ode_func_sindy(self, t, q_norm, drivers, A_low_c, B_low_c, A_mid_c, B_mid_c, A_high_c, B_high_c, sindy_tgt_col_1, pca_couplings, kp_th):
        ...
        qF_norm = self.move_column(np.copy(X_k_norm).T, 7, sindy_tgt_col_1).T
    ...
    def propagate_models(self, init_date, forward_propagation = 5):
        ...
        for chosen_basis_function in list(self.selected_bf_dict.keys()):
            ...
            solution_sindy = solve_ivp(
                ...
                args = (interpolated_drivers, A_sindy_joint_low_c, B_sindy_joint_low_c, A_sindy_joint_mid_c, B_sindy_joint_mid_c, A_sindy_joint_high_c, \
                                B_sindy_joint_high_c, self.sindy_tgt_col, self.pca_coupling, self.kp_th),
                method = 'RK45',
                t_eval = t_interval
            )
...
class rope_data_interpolator( PythonAtmosphere ):
    ...
    def interpolate_full_grid( self, timestamps: np.array, lla: np.array, forward_propagation: int = 3) -> tuple[float, float]:
        ...
        density_std = np.nanstd(density_models_values, axis = 1)
        density = np.nanmean(density_models_values, axis = 1)
