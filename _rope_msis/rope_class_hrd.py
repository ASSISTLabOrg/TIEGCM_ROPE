...
class rope_propagator:
    '''
    Class to store and propagate tyhe SINDY models based on TIEGCM physics based data
    '''
    def __init__( self, datapath: str = ".", drivers = None):
        #72: longitude intervals, 36: latitude intervals, 45: altitude intervals
        self.input_data_sindy = np.load(path.join(datapath, 'z_drivers_dataset_01_08_v2.npz'), allow_pickle=True)
        ...
        if drivers is None:
            ...
            self.original_drivers = self.input_data_sindy['drivers']
        ...
        'poly_all': poly3,
        'poly_exp': poly1 | exp1,
        'poly_sincos4': poly1 | sincos4,
        'poly_sincos7': poly1 | sincos7
        }
    ...
    def propagate_models(self, init_date, forward_propagation = 5):
        ...
        B_nl_dmd_discrete = input_data_models['models_dict']['nl-dmd']['plain']['ridge_parameter_0.00']
        ...
        for chosen_basis_function in list(self.selected_bf_dict.keys()):
            ...
            solution_sindy = solve_ivp(
                ...
                args = (interpolated_drivers, A_sindy_joint_c, B_sindy_joint_c, A_sindy_f10_c, B_sindy_f10_c, self.sindy_tgt_col, self.pca_coupling, \
                        self.kp_th),
                method = 'RK45',
                t_eval = t_interval
            )
...
class rope_data_interpolator( PythonAtmosphere ):
    ...
    def __init__( self, data: rope_propagator, earth_shape: BodyShape = None, sigma_point_value: float = 0.0 ):
        super().__init__()
    ...
    def interpolate( self, timestamps: np.array, lla: np.array, forward_propagation: int = 3) -> tuple[float, float]:
        ...
        for sindy_model in list(self.data.z_dict.keys()):
            sindy_interpolators_lst.append(interp1d(t.flatten(), self.data.z_dict[sindy_model], \
                kind='linear', axis=1, fill_value = "interpolate"))
        
        z_uncertainties_values_lst = [ 10 ** (np.sum(U0_interp * interpolator(T).T, axis = 1).reshape((-1, 1)) + \
