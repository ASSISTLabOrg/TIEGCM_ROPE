...
df_to_plot = outputs_df.loc[
    (outputs_df.datetime.dt.date == pd.to_datetime('2024-05-11').date()) &
    (outputs_df.datetime.dt.hour == 2)
]
ax.plot(df_to_plot.datetime, df_to_plot.ensemble_density)

ax.grid(True)

# Use tick_params for reliable label rotation and size
ax.tick_params(axis='x', labelrotation=25, labelsize=12)
ax.tick_params(axis='y', labelsize=12)

# If bold labels are important:
for label in ax.get_xticklabels() + ax.get_yticklabels():
    label.set_fontweight('bold')
...
outputs_df.loc[
    (outputs_df.datetime.dt.date == pd.to_datetime('2024-05-11').date()) &
    (outputs_df.datetime.dt.hour == 2) & (outputs_df.datetime.dt.minute >= 0) & (outputs_df.datetime.dt.minute <= 10)
].shape, outputs_df.loc[
    (outputs_df.datetime.dt.date == pd.to_datetime('2024-05-11').date()) &
    (outputs_df.datetime.dt.hour == 2) & (outputs_df.datetime.dt.minute >= 50) & (outputs_df.datetime.dt.minute <= 59)
].shape

interpolated_drivers_df2023b.loc[interpolated_drivers_df2023b.den > 0.6e-11].plot(y = 'lat')

interpolated_drivers_df2023b[(interpolated_drivers_df2023b.lat>87)]['lat'].max()

# +
msis_version = 2.1
omega = 1.6

def debias_emulator(start_date, bias_propagation):
    ...
    debias_dataset = grace_all.loc[start_debias:end_debias]
    debias_dataset['hour'] = debias_dataset.index.hour
    debias_dataset['hour_minute'] = debias_dataset.index.minute/60 + debias_dataset['hour']
    debias_dataset['hms'] = debias_dataset.index.second/3600 + debias_dataset.index.minute/60 + debias_dataset['hour']
    debias_dataset['t1'] = np.cos(np.pi*2.*debias_dataset['hms'].values/omega)
    debias_dataset['t2'] = np.sin(np.pi*2.*debias_dataset['hms'].values/omega)
    ...
    _, _, ensemble_density, _ = rope_density_interpolator.interpolate(timestamps, lla_array)
    debias_dataset['debias_ratio'] = np.abs(debias_dataset.grace_fo_density/ensemble_density)
    return debias_dataset 

def select_data(start_date, end_date, interpolator, msis_version, grace_dataset):

    grace_subset = grace_dataset.loc[start_date:end_date]

    timestamps = grace_subset.index.values
    latitude_values = grace_subset.lat.values # degrees
    local_time_values = grace_subset.lst.values # hours
    longitude_values = grace_subset.lon.values
    altitude_values = grace_subset.alt.values/1000. # km
    lla_array = np.vstack((latitude_values, local_time_values, altitude_values)).T.reshape((-1, 3))
    ...
    msis_rho = result[:, 0]
    accelerometer_density = grace_subset["grace_fo_density"].values
    orbital_mean_density = grace_subset["dens_mean"].values
    ...
    merged_drivers.sort_values('datetime', inplace=True)
    merged_drivers.ffill(inplace=True)
    merged_drivers.reset_index(drop=True, inplace=True)

    return timestamps, orbital_mean_density, accelerometer_density, ensemble_density, dmd_density, density_std, msis_rho, merged_drivers

#Latest Ridge parameters are [1., 1000, 10000, 100000]
selected_bf_dict = {
        'poly': 1.0,
        'poly17': 1000.,
        'poly12': 10.0, # Not able to recover after the storm
        'poly1357': 1000.0, 
        'poly_sincos4': 100.0, 
        'poly_sincos7': 1.0,
        'poly_exp1': 1.0,
...
end_date = "2023-01-05 09:00:00"
forward_propagation = 3

start_date = "2023-05-05 03:00:00"
end_date = "2023-05-08 12:00:00"
forward_propagation = 4

start_date = "2024-05-09 03:00:00"
end_date = "2024-05-14 03:00:00"
...
timestamps2023b, orbital_mean_density2023b, accelerometer_density2023b, ensemble_density2023b, dmd_density2023b, \
    density_std2023b, msis_rho2023b, interpolated_drivers_df2023b = \
        select_data(start_date, end_date, rope_density, msis_version, grace_all)

debias_dataset = debias_emulator(start_date, bias_propagation)
debias_ratio_df = debias_dataset.groupby(['t1', 't2']).agg(mean_ratio = ('debias_ratio', 'mean')).reset_index().copy()
debias_density_df = pd.DataFrame({'datetime': timestamps2023b, 
    'hour_minute': pd.to_datetime(timestamps2023b).hour + pd.to_datetime(timestamps2023b).minute/60., 
        'hms': pd.to_datetime(timestamps2023b).second/3600 + pd.to_datetime(timestamps2023b).minute/60 + pd.to_datetime(timestamps2023b).hour,
            'den': ensemble_density2023b})
debias_density_df['t1'] = np.cos(np.pi*2.*debias_density_df['hms'].values/omega)
debias_density_df['t2'] = np.sin(np.pi*2.*debias_density_df['hms'].values/omega)

debias_density_df = pd.merge(debias_density_df, debias_ratio_df, on=['t1', 't2'], how = 'left')
debias_density_df['debiased_density'] = debias_density_df['den'] * debias_density_df['mean_ratio']
debiased_ensemble_density2023b = debias_density_df.debiased_density.values


fig, ax = plt.subplots(1, 1, figsize=(20, 6), sharex=False)

# Subplot 1 — Density comparison
ax.plot(timestamps2023b, accelerometer_density2023b, label="GRACE-FO acceleromenter density", color='tab:blue', linewidth=2)
ax.plot(timestamps2023b, msis_rho2023b, label="NRL-MSIS 2.1", color='orange', linewidth=1.0)
ax.plot(timestamps2023b, ensemble_density2023b, label="Ensemble density", color='black', linewidth=1.)
ax.plot(timestamps2023b, debiased_ensemble_density2023b, label="Debiased ensemble density", color='green', linewidth=1.)
# ax.fill_between(timestamps2023b, debiased_ensemble_density2023b - density_std2023b, debiased_ensemble_density2023b + density_std2023b,
#                    color='grey', alpha=0.2, label="Ensemble confidence interval")
# ax.plot(timestamps2023b, dmd_density2023b, label="DMD", color='violet', linewidth=1.0)
...
# Labels and legend
ax.set_ylabel(r"$\rho$ (kg/m$^3$)", fontsize=14)
ax.set_title("Neutral density comparison – GRACE-FO vs Ensemble vs NRL-MSIS 2.1", fontsize=16)
ax.legend(loc='upper left', bbox_to_anchor=(1.05, 1), borderaxespad=0.)

ax12 = ax.twinx()
ax12.plot(interpolated_drivers_df2023b.datetime, interpolated_drivers_df2023b.kp, label="$K_p$", color='tab:red', linestyle='--', linewidth=1)
...
plt.xticks(rotation=25)
ax.set_ylim(0.15e-12, 1.3e-11)
plt.tight_layout()
# plt.savefig('./imgs/grace_fo_vs_rope_ensemble_2024_debiased.png', dpi=300, bbox_inches='tight')
plt.show()
# -
...
debias_dataset['t1'] = np.cos(np.pi*2.*debias_dataset['hms'].values/1.5)
debias_dataset['t2'] = np.sin(np.pi*2.*debias_dataset['hms'].values/1.5)

fig, ax = plt.subplots(1, 1, figsize=(20, 6), sharex=False)

# Subplot 1 — Density comparison
ax.plot(debias_dataset.iloc[:1000, :].index, debias_dataset.iloc[:1000, :]['t1'], label="t1", color='tab:blue', linewidth=1.0)
ax.plot(debias_dataset.iloc[:1000, :].index, debias_dataset.iloc[:1000, :]['t2'], label="t1", color='tab:blue', linewidth=1.0)
...
# Subplot 1 — Density comparison
ax.plot(timestamps2023a, accelerometer_density2023a, label="GRACE-FO acceleromenter density", color='tab:blue', linewidth=1.0)
# ax[0].plot(timestamps2023a, msis_rho2023a, label="NRL-MSIS 2.1", color='red', linewidth=1.0)
ax.plot(timestamps2023a, ensemble_density2023a, label="Ensemble density", color='black', linewidth=1.)
ax.fill_between(timestamps2023a, ensemble_density2023a - density_std2023a, ensemble_density2023a + density_std2023a,
                   color='grey', alpha=0.2, label="Ensemble confidence interval")
ax.plot(timestamps2023a, dmd_density2023a, label="DMD", color='green', linewidth=1.0)
...
ax12 = ax.twinx()
ax12.plot(interpolated_drivers_df2023a.datetime, interpolated_drivers_df2023a.kp, label="$K_p$", color='tab:red', linestyle='--', linewidth=1)
...
plt.xticks(rotation=25)
plt.tight_layout()
plt.savefig('./imgs/grace_fo_vs_rope_ensemble_2023a.png', dpi=300, bbox_inches='tight')
plt.show()
...
# Subplot 1 — Density comparison
ax.plot(timestamps2023b, accelerometer_density2023b, label="GRACE-FO acceleromenter density", color='tab:blue', linewidth=1.0)
# ax[0].plot(timestamps2023b, msis_rho2023b, label="NRL-MSIS 2.1", color='red', linewidth=1.0)
# ax.plot(timestamps2023b, ensemble_density2023b, label="Ensemble density", color='black', linewidth=1.5)
ax.plot(timestamps2023b, debiased_ensemble_density2023b, label="Ensemble density", color='green', linewidth=1.5)
ax.fill_between(timestamps2023b, ensemble_density2023b - density_std2023b, ensemble_density2023b + density_std2023b,
                   color='grey', alpha=0.2, label="Ensemble confidence interval")
# ax.plot(timestamps2023b, dmd_density2023b, label="DMD", color='green', linewidth=1.0)
...
plt.xticks(rotation=25)
plt.tight_layout()
plt.savefig('./imgs/grace_fo_vs_rope_ensemble_2023b.png', dpi=300, bbox_inches='tight')
plt.show()
...
# Subplot 1 — Density comparison
ax.plot(timestamps2024, accelerometer_density2024, label="GRACE-FO acceleromenter density", color='tab:blue', linewidth=1.0)
# ax[0].plot(timestamps2024, msis_rho2024, label="NRL-MSIS 2.1", color='red', linewidth=1.0)
ax.plot(timestamps2024, ensemble_density2024, label="Ensemble density", color='black', linewidth=1.)
ax.fill_between(timestamps2024, ensemble_density2024 - density_std2024, ensemble_density2024 + density_std2024,
                   color='grey', alpha=0.2, label="Ensemble confidence interval")
# ax[0].plot(timestamps2024, dmd_density2024, label="DMD", color='green', linewidth=1.0)
...
ax12 = ax.twinx()
ax12.plot(interpolated_drivers_df2024.datetime, interpolated_drivers_df2024.kp, label="$K_p$", color='tab:red', linestyle='--', linewidth=1)
...
plt.xticks(rotation=25)
plt.tight_layout()
plt.savefig('./imgs/grace_fo_vs_rope_ensemble_2024.png', dpi=300, bbox_inches='tight')
plt.show()

# +
fig, ax = plt.subplots(3, 1, figsize=(20, 13), sharex=False)

# Subplot 1 — Density comparison
ax[0].plot(timestamps2023a, accelerometer_density2023a, label="GRACE-FO acceleromenter density", color='tab:blue', linewidth=1.0)
# ax[0].plot(timestamps2023a, msis_rho2023a, label="NRL-MSIS 2.1", color='red', linewidth=1.0)
ax[0].plot(timestamps2023a, ensemble_density2023a, label="Ensemble density", color='black', linewidth=1.)
ax[0].fill_between(timestamps2023a, ensemble_density2023a - density_std2023a, ensemble_density2023a + density_std2023a,
                   color='grey', alpha=0.2, label="Ensemble confidence interval")
# ax[0].plot(timestamps2023a, dmd_density2023a, label="DMD", color='green', linewidth=1.0)


# Labels and legend
ax[0].set_ylabel(r"$\rho$ (kg/m$^3$)", fontsize=14)
ax[0].set_title("Neutral density comparison – GRACE-FO vs Ensemble vs NRL-MSIS 2.1", fontsize=16)
...
ax[1].plot(timestamps2023b, accelerometer_density2023b, label="GRACE-FO acceleromenter density", color='tab:blue', linewidth=1)
ax[1].plot(timestamps2023b, msis_rho2023b, label="NRL-MSIS 2.1", color='red', linewidth=1)
ax[1].plot(timestamps2023b, ensemble_density2023b, label="Ensemble density", color='black', linewidth=1.)
ax[1].fill_between(timestamps2023b, ensemble_density2023b - density_std2023b, ensemble_density2023b + density_std2023b,
                   color='grey', alpha=0.2, label="Ensemble confidence interval")
# ax[1].plot(timestamps2023b, dmd_density2023b, label="DMD", color='green', linewidth=1)
ax[1].legend(loc='upper left', bbox_to_anchor=(1.05, 1), borderaxespad=0.)

ax22 = ax[1].twinx()
ax22.plot(interpolated_drivers_df2023b.datetime, interpolated_drivers_df2023b.kp, label="$K_p$", color='tab:red', linestyle='--', linewidth=1)



ax[2].plot(timestamps2024, accelerometer_density2024, label="GRACE-FO acceleromenter density", color='tab:blue', linewidth=1)
ax[2].plot(timestamps2024, msis_rho2024, label="NRL-MSIS 2.1", color='red', linewidth=1)
ax[2].plot(timestamps2024, ensemble_density2024, label="Ensemble density", color='black', linewidth=1.)
ax[2].fill_between(timestamps2024, ensemble_density2024 - density_std2024, ensemble_density2024 + density_std2024,
                   color='grey', alpha=0.2, label="Ensemble confidence interval")
# ax[2].plot(timestamps2024, dmd_density2024, label="DMD", color='green', linewidth=1)

ax[2].legend(loc='upper left', bbox_to_anchor=(1.05, 1), borderaxespad=0.)

ax22 = ax[2].twinx()
ax22.plot(interpolated_drivers_df2024.datetime, interpolated_drivers_df2024.kp, label="$K_p$", color='tab:red', linestyle='--', linewidth=1)
...
plt.xticks(rotation=25)
plt.tight_layout()
plt.savefig('./imgs/grace_fo_vs_rope_ensemble_no_bias_shortened.png', dpi=300, bbox_inches='tight')
plt.show()
...
ax[0].plot(timestamps2023a, accelerometer_density2023a, label="GRACE-FO acceleromenter density", color='tab:blue', linewidth=1.0)
# ax[0].plot(timestamps2023a, msis_rho2023a, label="NRL-MSIS 2.1", color='red', linewidth=1.0)
ax[0].plot(timestamps2023a, debiased_ensemble_density2023a, label="Ensemble density", color='black', linewidth=1.)
ax[0].fill_between(timestamps2023a, ensemble_density2023a - density_std2023a, ensemble_density2023a + density_std2023a,
                   color='grey', alpha=0.2, label="Ensemble confidence interval")
ax[0].plot(timestamps2023a, dmd_density2023a, label="DMD", color='green', linewidth=1.0)
...
ax[1].plot(timestamps2023b, accelerometer_density2023b, label="GRACE-FO acceleromenter density", color='tab:blue', linewidth=1)
# ax[1].plot(timestamps2023b, msis_rho2023b, label="NRL-MSIS 2.1", color='red', linewidth=1)
ax[1].plot(timestamps2023b, debiased_ensemble_density2023b, label="Ensemble density", color='black', linewidth=1.)
ax[1].fill_between(timestamps2023b, ensemble_density2023b - density_std2023b, ensemble_density2023b + density_std2023b,
                   color='grey', alpha=0.2, label="Ensemble confidence interval")
ax[1].plot(timestamps2023b, dmd_density2023b, label="DMD", color='green', linewidth=1)
ax[1].legend(loc='upper left', bbox_to_anchor=(1.05, 1), borderaxespad=0.)
...
ax[2].plot(timestamps2024, accelerometer_density2024, label="GRACE-FO acceleromenter density", color='tab:blue', linewidth=1)
# ax[2].plot(timestamps2024, msis_rho2024, label="NRL-MSIS 2.1", color='red', linewidth=1)
ax[2].plot(timestamps2024, debiased_ensemble_density2024, label="Ensemble density", color='black', linewidth=1.)
ax[2].fill_between(timestamps2024, ensemble_density2024 - density_std2024, ensemble_density2024 + density_std2024,
                   color='grey', alpha=0.2, label="Ensemble confidence interval")
ax[2].plot(timestamps2024, dmd_density2024, label="DMD", color='green', linewidth=1)
...
plt.xticks(rotation=25)
plt.tight_layout()
plt.savefig('./imgs/grace_fo_vs_rope_ensemble_with_bias_shortened.png', dpi=300, bbox_inches='tight')
plt.show()
