...
def load_h5(filename: str, return_sw: bool) -> np.ndarray:
    ...
    :rtype: np.ndarray
    """
    keys = ["den", "F107", "Kp"] if return_sw else ["den"]
    output = {}
...
def preprocess_from_h5(years):
    drivers_lst = []
    state_lst = []

    for year in years:
        data_dict = {}
        data_dict = load_h5(f"./data/inputs/{year}_TIEGCM_Density.h5", return_sw=False)
        data_dict['year'] = year
        print(data_dict['den'].shape)
        den = data_dict['den'].reshape((data_dict['den'].shape[0], -1))
        state_df = pd.DataFrame(den)
        state_df.columns = ['x_' + str(name) for name in state_df.columns]
        drivers_dict = {'year': data_dict['year'], 'utc': data_dict['UTC'], 'doy': data_dict['doy'], 'f107': data_dict['F107'], 'kp': data_dict['Kp']}
        drivers_df = pd.DataFrame(drivers_dict)
        drivers_lst.append(drivers_df)
        state_lst.append(state_df)

    state_data = np.concatenate(state_lst).T
    drivers_data = np.concatenate(drivers_lst).T
    return state_data, drivers_data

def preprocess_from_h5_ext(years):
    drivers_lst = []
    state_lst = []

    for year in years:
        data_dict = {}
        data_dict = load_h5(f"./data/ext_inputs/tiegcm_ext_yearly/{year}_density_data.h5", return_sw=False)
        data_dict['year'] = year
        print(data_dict['DEN'].shape)
        den = np.log10(data_dict['DEN'].reshape((data_dict['DEN'].shape[0], -1)))
        
        timestamps = pd.DataFrame(pd.date_range(start="2025-01-01 00:00:00", periods=len(den), freq="h"), columns = ['date']) 
        hours = timestamps.date.dt.hour.values
        doy = timestamps.date.dt.dayofyear
        data_dict['UTC'] = hours
        data_dict['doy'] = doy
        
        state_df = pd.DataFrame(den)
        state_df.columns = ['x_' + str(name) for name in state_df.columns]
        drivers_dict = {'year': data_dict['year'], 'utc': data_dict['UTC'], 'doy': data_dict['doy'], 'f107': data_dict['f107d'], 'kp': data_dict['Kp']}
        drivers_df = pd.DataFrame(drivers_dict)
        drivers_lst.append(drivers_df)
        state_lst.append(state_df)
...
def gather_data_ext(train_years = [2000, 2001, 2002], test_years = [2003], is_holdout = True):
    if is_holdout:
        print(f'Downloading {len(train_years)} years')
        state_data, drivers_data = preprocess_from_h5_ext(train_years)
...
def gather_data(train_years = [2000, 2001, 2002], test_years = [2003], is_holdout = True):
    if is_holdout:
        print(f'Downloading {len(train_years)} years')
        state_data, drivers_data = preprocess_from_h5(train_years)
        dataset_dict = {'state_data':state_data, 'drivers_data':drivers_data}
        return dataset_dict        
    else:
        print(f'Downloading {len(train_years)} years for train')
...
def create_library_functions(x, functions_dictionary, feature_names):
    ...
    len(feature_names_interactions_lst)
    ...
    left_permutation = [p[0] for p in combs]
    right_permutation = [p[1] for p in combs]
    theta = np.concatenate([np.ones(arr.shape[0]).reshape((-1, 1)), arr, arr[:, left_permutation] * arr[:, right_permutation]], axis = 1)
